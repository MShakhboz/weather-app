...
const data = async (pattern, param) => {
    try {
        const request = await axios.get(
            `https://community-open-weather-map.p.rapidapi.com/${pattern}`,
            {
                params: {...param},
                headers: {
                    'X-RapidAPI-Host':
                    ...
                    'X-RapidAPI-Key':
                        '573bc962acmsheafdfc22464aa1fp18bff7jsna3160e411265',
                },
            },
        );
...
export function* getForecastDailyWeather(date) {
    try {
        const weatherInfo = yield call(data, 'forecast/daily', {
            q: 'London,uk',
            units: 'metric',
        });
        yield put({
            type: actionsReducers.LOAD_DAILY_FORECAST,
            payload: weatherInfo.list,
        });
    } catch (e) {
        console.log(e);
    }
