...
const WeatherScreen = ({getForecastDailyWeather, weatherInfo}) => {
    ...
    useEffect(() => {
        getForecastDailyWeather(date);
    }, [date, getForecastDailyWeather]);
    console.log('Weather screen', weatherInfo);
    const currentDay = Object.keys(weatherInfo).length ? weatherInfo[0] : null;
    console.log(currentDay);
    const forecast = Object.keys(weatherInfo).length
        ? weatherInfo.splice(1)
        : null;
...
const mapDispatchToProps = dispatch => {
    return {
        getForecastDailyWeather: date =>
            dispatch({type: actions.DAILY_FORECAST.REQUEST}, date),
    };
};
